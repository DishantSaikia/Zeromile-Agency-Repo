...
const base =
  ...
  "inline-flex items-center justify-center gap-2 font-medium tracking-tight bg-whatsapp text-on-whatsapp hover:bg-whatsapp-dark active:bg-whatsapp-dark transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] cursor-pointer active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-whatsapp-dark focus-visible:ring-offset-2 focus-visible:ring-offset-bg";
...
export function WhatsAppButton({
  ...
  if (variant === "floating") {
    return (
      <a
        ...
        className={`${base} fixed z-50 h-14 w-14 rounded-full shadow-[0_12px_32px_-12px_rgba(0,0,0,0.55)] bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-4 md:bottom-6 md:right-6 ${className}`}
      >
  ...
  return (
    <a
      ...
      className={`${base} rounded-[var(--radius-inner)] ${sizeClasses} ${className}`}
    >
